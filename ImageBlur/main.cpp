...
//__constant__ float d_filterConstant[81];
...
int main(int argc, char const *argv[])
{
   ...
   cv::Mat imrgba, o_img, color_channel; 
   ...
   color_channel.create(img.rows, img.cols, CV_8UC1);
   ...
   //h_red = (unsigned char*)color_channel.ptr<unsigned char>(0); // test channel
   ...
   checkCudaErrors(cudaMalloc((void**)&d_red, sizeof(char)*numPixels));
   checkCudaErrors(cudaMalloc((void**)&d_red_blurred, sizeof(char)*numPixels));
   checkCudaErrors(cudaMalloc((void**)&d_blue, sizeof(char)*numPixels));
   checkCudaErrors(cudaMalloc((void**)&d_blue_blurred, sizeof(char)*numPixels));
   checkCudaErrors(cudaMalloc((void**)&d_green, sizeof(char)*numPixels));
   checkCudaErrors(cudaMalloc((void**)&d_green_blurred, sizeof(char)*numPixels));
   ...
   //checkCudaErrors(cudaMemcpy(h_red, d_red, sizeof(unsigned char)*numPixels, cudaMemcpyDeviceToHost)); // for bug checking   
